...
import json
import math
from utils import load_and_cache_examples, set_seed, get_f1, Tokenizer, FGM
...
MODEL_CLASSES = {
    "bert": (BertConfig, BertForMaskedLM, Tokenizer),
...
#                                 str(args.train_language),
        ...
        for step in bar:
            ...
            loss_adv = model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_mask, masked_lm_labels=output_ids)[0]
            if args.n_gpu > 1:
...
#                 scheduler.step()
            ...
            if args.do_eval and (step + 1) %(args.eval_steps*args.gradient_accumulation_steps)==0:
                ...
                for file in file_list:
                    ...
                    for input_ids, input_mask, segment_ids, output_ids in tqdm(dev_dataloader):
                        ...
                        with torch.no_grad():
                            tmp_eval_loss, logits= model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_mask, masked_lm_labels=output_ids)
                        if args.n_gpu > 1:
    ...
    if args.do_test:
        ...
        for input_ids, input_mask, segment_ids, output_ids in tqdm(dev_dataloader):
            ...
            with torch.no_grad():
                tmp_eval_loss = model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_mask, masked_lm_labels=output_ids)[0].detach().cpu().numpy()
            if args.n_gpu > 1:
                tmp_eval_loss = tmp_eval_loss.mean() # mean() to average on multi-gpu.
            if args.gradient_accumulation_steps > 1:
                tmp_eval_loss = tmp_eval_loss / args.gradient_accumulation_steps
